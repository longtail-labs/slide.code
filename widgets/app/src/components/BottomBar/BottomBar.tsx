...
const BottomBar = () => {
  ...
  // Create fake radio station
  const createFakeRadioStation = (): SomaFmChannel => {
    const randomListeners = Math.floor(Math.random() * 501) + 100 // 100-600
    return {
      id: 'aquamarine-radio',
      title: 'Aquamarine Radio',
      description: 'Addison Rae\'s debut album "Addison" - 12 tracks of dreamy pop magic',
      dj: 'DJ Addison',
      genre: 'Pop/Dance-Pop',
      image: '',
      largeimage: '',
      ...
      playlists: [], // Empty since we'll handle this directly
      preroll: [],
      listeners: randomListeners.toString(),
      lastPlaying: 'Aquamarine Dreams'
    }
  }
  ...
  useEffect(() => {
    const fetchChannels = async () => {
      try {
        ...
        // Add our fake radio station
        const fakeStation = createFakeRadioStation()
        const allChannels = [fakeStation, ...fetchedChannels]

        // Sort channels by listener count (highest first)
        const sortedChannels = allChannels.sort((a: SomaFmChannel, b: SomaFmChannel) => {
        ...
        // Try to find Groove Salad first, then Aquamarine Radio, then default to first
        const grooveSaladIndex = sortedChannels.findIndex(
          (c: SomaFmChannel) => c.id === 'groovesalad'
        )
        const aquamarineIndex = sortedChannels.findIndex(
          (c: SomaFmChannel) => c.id === 'aquamarine-radio'
        )

        if (grooveSaladIndex !== -1) {
          setCurrentChannelIndex(grooveSaladIndex)
        } else if (aquamarineIndex !== -1) {
          setCurrentChannelIndex(aquamarineIndex)
        } else {
  ...
  useEffect(() => {
    ...
    const fetchPlaylist = async () => {
      ...
      // Handle fake radio station
      if (channel.id === 'aquamarine-radio') {
        const directStreamUrl =
          'https://pub-613070f46710422c971ce0da4ccbc79d.r2.dev/aquamarine_radio.mp3'
        setStreamServers([directStreamUrl])
      ...
      // Handle regular SomaFM channels
