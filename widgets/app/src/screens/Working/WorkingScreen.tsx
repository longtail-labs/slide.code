...
import { useTaskWithMessages, useAppReadyRef } from '@slide.code/clients'
import type { TaskWithMessages, FileComment } from '@slide.code/schema'
...
interface WorkingScreenContextType {
  ...
  commentsData: FileComment[]
  setCommentsData: (comments: FileComment[]) => void
  clearAllComments: () => void
  registerClearComments: (callback: () => void) => void
}
...
const WorkingScreenProvider = ({
  ...
  const [commentsData, setCommentsData] = useState<FileComment[]>([])
  const clearCommentsCallbackRef = React.useRef<(() => void) | null>(null)

  const registerClearComments = (callback: () => void) => {
    clearCommentsCallbackRef.current = callback
  }

  const clearAllComments = () => {
    if (clearCommentsCallbackRef.current) {
      clearCommentsCallbackRef.current()
    }
  }
...
const WorkingScreen = () => {
  ...
  const [appReadyState, setAppReadyState, updateAppReadyState] = useAppReadyRef()

  console.log('WorkingScreen TASK', task, isLoading, error)

  console.log('WorkingScreen APP READY STATE', appReadyState)
  ...
  return (
    <WorkingScreenProvider task={task}>
      <motion.div
        className="flex flex-col h-full w-full overflow-hidden bg-background select-text"
        variants={variants}
        initial="initial"
        animate="animate"
        exit="exit"
