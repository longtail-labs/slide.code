...
export function DiffViewer({ diffText, isLoading, error }: DiffViewerProps) {
  ...
  const { setCommentsCount, setCommentsData, registerClearComments } = useWorkingScreenContext()

  // Parse diff files first
  const files = useMemo(() => {
  ...
  // Create a mapping from change key to file and line info
  const changeKeyToFileInfo = useMemo(() => {
    const mapping: Record<string, { filePath: string; lineNumber?: number; changeType: string }> =
      {}
    ...
    files.forEach((file) => {
      ...
      file.hunks.forEach((hunk) => {
        hunk.changes.forEach((change) => {
          ...
          // Handle different change types with correct property access
  ...
  // Create FileComment objects from comments
  const fileComments = useMemo(() => {
    return Object.entries(comments)
      .map(([key, comment]) => {
        const fileInfo = changeKeyToFileInfo[key]
      ...
      .filter(Boolean) as Array<{ filePath: string; comment: string; lineNumber?: number }>
  }, [comments, changeKeyToFileInfo])

  // Update comments count and data in context whenever comments change
  useEffect(() => {
    setCommentsCount(Object.keys(comments).length)
    setCommentsData(fileComments)
  }, [comments, fileComments, setCommentsCount, setCommentsData])

  // Register clear comments callback
  ...
  useEffect(() => {
    registerClearComments(clearAllComments)
  }, [registerClearComments, clearAllComments])
