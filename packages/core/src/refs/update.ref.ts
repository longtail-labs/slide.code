import { Effect, Fiber, Schedule, SubscriptionRef, Stream } from 'effect'
...
/**
 * UpdateRef service for tracking application update state using SubscriptionRef
 */
export class UpdateRef extends Effect.Service<UpdateRef>()('UpdateRef', {
  dependencies: [PubSubClient.Default],
  scoped: Effect.gen(function* () {
    yield* Effect.logInfo('[UpdateRef] 🚀 Creating UpdateRef')

    // Create the subscription ref
    const ref = yield* SubscriptionRef.make(initialUpdateState)
    ...
    /**
     * Check if an update is available without updating the ref
     */
    const isUpdateAvailableCheck = Effect.gen(function* () {
      try {
        yield* Effect.logInfo('[UpdateRef] Checking if update is available...')
        ...
        // Get latest version from server
        const latestVersion = yield* getLatestVersion(updateSiteURL)

        // Check if update is available
        const updateAvailable = isUpdateAvailable(currentVersion, latestVersion)

        yield* Effect.logInfo(
          `[UpdateRef] Update check result: Current: ${currentVersion}, Latest: ${latestVersion}, Update available: ${updateAvailable}`
        )
    ...
    // Return an object with the ref, changes stream, and methods
    return {
      // Expose the changes stream directly
      changes: ref.changes,
      initialize,
      checkForUpdates: checkForUpdatesInternal,
      triggerUpdateUI: triggerUpdateUIInternal,
      isUpdateAvailableCheck: isUpdateAvailableCheck,
...
/**
 * Check if an update is available (exposed for direct invocation)
 */
export const updateAvailable = Effect.gen(function* () {
  const updateRef = yield* UpdateRef
  return yield* updateRef.isUpdateAvailableCheck
})
