  ...
  Effect.sync(() => ({
    ...
    loadComprehensiveStats: () =>
      Effect.gen(function* () {
        ...
        // Log the data counts for basic monitoring
        log.info('[CcusageService] 📊 Loaded usage data:', {
          dailyEntries: rawDailyData.length,
          sessionEntries: rawSessionData.length
        })
        ...
        log.info('[CcusageService] ✅ Stats loaded successfully:', {
          totalCost: result.tokenTotals.totalCost,
          totalTokens: result.tokenTotals.totalTokens,
          modelsCount: result.modelsUsed.length
        })
